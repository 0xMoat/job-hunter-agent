...
1. **先判断是否应当结束**：**仅**在以下情况返回 `Response`：
   - 原计划所有步骤都已出现在 "Steps already executed" 中（即剩余步骤为空）；
   - 客观上无法再推进（连续失败、缺少工具、信息不足且无法补齐）。
2. **其它所有情况**都返回 `Plan`，并且只列剩余步骤。哪怕你主观觉得"已经做了不少"、"用户八成满意了"、或"某张卡片的关键分析都齐了"——只要 plan 里还有未执行的步骤，就必须继续执行完，不要自作主张提前终止。
3. 如果剩余步骤确实有问题（依赖的前置失败、产生了红旗等），可以**修改或删减**剩余 plan，但依然返回 `Plan`，不要返回 `Response`。
